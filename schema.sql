...
CREATE TABLE IF NOT EXISTS horas_extras (
  ...
  -- Cadena de autorización asignada al momento del registro. Se guarda aquí
  -- (y no sólo en usuarios) para que el histórico conserve quién DEBÍA
  -- autorizar, aunque después le cambien de jefe al empleado.
  jefe_num        VARCHAR(20),
  ...
  -- v3 · Quién levantó la solicitud. El jefe solicita para su personal;
  -- 'auto' marca los registros heredados que capturó el propio empleado.
  lote_id            TEXT,
  origen             TEXT DEFAULT 'jefe',
  solicitante_num    VARCHAR(20),
  solicitante_nombre TEXT,
  solicitante_ts     TIMESTAMPTZ,
  rechazado_por      TEXT,
  -- v3 · Nivel 3: consentimiento del trabajador (firma electrónica)
  acept_emp         TEXT DEFAULT 'na',
  acept_emp_ts      TIMESTAMPTZ,
  acept_emp_ip      TEXT,
  acept_emp_ua      TEXT,
  acept_emp_leyenda TEXT,
  acept_emp_ley_ver TEXT,
  acept_emp_hash    TEXT,
  acept_emp_nota    TEXT,
  estado_final    TEXT         DEFAULT 'pendiente'
);

-- Migración segura para bases creadas con la v1 / v2
...
-- CHECKs con nombre propio: así se pueden reemplazar sin recrear la tabla
...
-- ──────────────────────────────────────────────────────────────────
-- TABLA: sabados_personal  (v3)
--
-- Un renglón por persona convocada. La columna personal (JSONB) de
-- arriba se conserva por compatibilidad, pero la fuente de verdad es
-- esta tabla: cada quien acepta o rechaza por separado y su firma
-- electrónica se guarda en su propio renglón.
-- ──────────────────────────────────────────────────────────────────
...
--
-- v3: el índice cubre únicamente lo que sigue vivo. Si el trabajador
-- rechaza la solicitud o ésta vence sin respuesta, el jefe debe poder
-- levantar otra ese mismo día; con el índice anterior quedaba bloqueado.
