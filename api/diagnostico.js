...
const COLUMNAS = {
  ...
  sabados_laborados: ['id','ts','fecha_sabado','jefe_num','jefe_nombre','ubicacion','depto','personal',
                      ...
                      'gerente_num','auth_gerente','auth_gerente_num','auth_gerente_ts'],
  ...
  try {
    ...
    const cols = await sql`
      ...
      WHERE table_schema = 'public'
        AND table_name IN ('usuarios','horas_extras','sabados_laborados','notificaciones')`;
    ...
    for (const [tabla, esperadas] of Object.entries(COLUMNAS)) {
      ...
      if (faltan.length) {
        problemas.push(`En "${tabla}" faltan las columnas: ${faltan.join(', ')}. Ejecuta migracion-v2.1.sql en Neon.`);
      }
    }
    ...
    const claves = ['idx_he_num_fecha','idx_he_fecha','uq_he_emp_dia','idx_usr_jefe','idx_usr_gerente'];
    const sinIdx = claves.filter(i => !nombres.has(i));
    info.indices = sinIdx.length ? `faltan: ${sinIdx.join(', ')}` : 'completos';
    if (sinIdx.length) {
      problemas.push(`Faltan índices (${sinIdx.join(', ')}). El sistema funciona pero el monitor será lento. Ejecuta migracion-v2.1.sql.`);
    }
    ...
    try {
      ...
      problemas.push('No se pudo insertar un registro de prueba: ' + e.message + ' → ejecuta migracion-v2.1.sql en Neon.');
    }
