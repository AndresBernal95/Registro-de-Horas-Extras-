...
/* Se ignoran los avisos que el navegador emite por un código HTTP de
   error: varias pruebas provocan un 409 a propósito (candado de un
   registro por día). Lo que aquí importa son los errores de
   JavaScript, no las respuestas que el servidor devuelve bien. */
pg.on('console', m => {
...
console.log('\n══ 3. BUCLE INFINITO EN "MIS HORAS" (el bug crítico de la v2.2) ══');
...
check('v3: el operativo YA NO puede capturar horas', (await pg.locator('.hour-btn').count()) === 0);
check('v3: ve la bandeja de solicitudes por responder',
  (await pg.locator('#fPend').textContent()||'').includes('Solicitudes por responder'));
check('El operario sólo ve su pestaña', !(await pg.locator('#tabsBar').isVisible()));
check('Sin overlay pegado', !(await overlayVisible()));

console.log('\n══ 4. CATEGORÍAS ADMINISTRATIVAS Y 5 PORQUÉS (en la solicitud del jefe) ══');
/* El árbol de causa raíz se mudó al modal con el que el jefe levanta
   la solicitud. Las mismas comprobaciones, en su lugar nuevo. */
await pg.click('.btn-logout'); await pg.waitForTimeout(300);
await pg.fill('#loginNum','8197'); await pg.fill('#loginPass','Jefe2026');
await pg.click('#btnLogin'); await pg.waitForSelector('#appScreen',{state:'visible'});
await pg.waitForTimeout(600);
await pg.click('[data-tab="registros"]'); await pg.waitForTimeout(900);
check('El jefe tiene el botón para solicitar horas a su personal',
  await pg.locator('[data-act="abrirModalHE"]').count() > 0);
await pg.click('[data-act="abrirModalHE"]');
await pg.waitForSelector('#modalHE:not(.hidden)'); await pg.waitForTimeout(700);

/* Se usa al empleado 101: el 140 ya trae una solicitud viva de hoy en
   los datos de prueba, y el candado de un registro por día la
   rechazaría antes de llegar a lo que aquí se quiere comprobar. */
await pg.selectOption('#fh_personal_sel','101');
await pg.click('[data-act="agregarPersonalHE"]'); await pg.waitForTimeout(250);
check('Se agrega personal con sus horas', (await pg.locator('.pers-fila').count()) === 1);
check('Cada renglón trae su selector de horas',
  await pg.locator('.pers-fila select[data-hepers="101"]').isVisible());

check('Existe el grupo visual "Administrativas"',
  (await pg.locator('#heArbol').textContent()||'').includes('Administrativas'));
...
const textoArbol = await pg.locator('#heArbol').textContent();
for(const c of adminEsperadas) check(`Categoría presente: ${c}`, textoArbol.includes(c));

await pg.click('#heArbol button[data-lvl="cat"][data-val="junta_jefe"]'); await pg.waitForTimeout(220);
check('Paso 1 registra la categoría', (await pg.locator('#heArbol').textContent()).includes('¿Con quién fue la reunión?'));
await pg.locator('#heArbol button[data-lvl="p1"]').first().click(); await pg.waitForTimeout(220);
await pg.locator('#heArbol button[data-lvl="p2"]').first().click(); await pg.waitForTimeout(220);
check('Llega al paso de causa raíz', (await pg.locator('#heArbol').textContent()).includes('Causa raíz'));
await pg.locator('#heArbol button[data-lvl="p3"]').first().click(); await pg.waitForTimeout(300);
check('Muestra el resumen de causa raíz identificada', await pg.locator('#heResumen .causa-ok').isVisible());

console.log('\n══ 5. NO SE PIERDE EL ESTADO AL CAMBIAR LAS HORAS ══');
await pg.selectOption('.pers-fila select[data-hepers="101"]','3'); await pg.waitForTimeout(350);
check('El árbol de causa raíz se conserva al cambiar las horas',
  await pg.locator('#heResumen .causa-ok').isVisible());
check('El acumulado proyectado se recalcula',
  (await pg.locator('#hePersonalList').textContent()||'').includes('sem:'));

console.log('\n══ 6. ENVÍO DEL LOTE Y CANDADO DE UN REGISTRO POR DÍA ══');
await pg.click('[data-act="guardarHE"]'); await pg.waitForTimeout(1800);
check('Confirma el envío con un aviso', (await pg.locator('#toastWrap').textContent()||'').length>0);
const modalCerrado = await pg.locator('#modalHE.hidden').count() === 1;
check('Se cierra el modal al enviar', modalCerrado,
  modalCerrado ? '' : 'error: ' + (await pg.locator('#modalHEErr').textContent()||''));
check('Sin overlay pegado tras enviar', !(await overlayVisible()));
if(!modalCerrado){ await pg.click('[data-act="cerrarModalHE"]'); await pg.waitForTimeout(300); }

await pg.click('[data-act="abrirModalHE"]');
await pg.waitForSelector('#modalHE:not(.hidden)'); await pg.waitForTimeout(700);
await pg.selectOption('#fh_personal_sel','140');
await pg.click('[data-act="agregarPersonalHE"]'); await pg.waitForTimeout(200);
await pg.click('#heArbol button[data-lvl="cat"][data-val="junta_jefe"]'); await pg.waitForTimeout(200);
await pg.locator('#heArbol button[data-lvl="p1"]').first().click(); await pg.waitForTimeout(200);
await pg.locator('#heArbol button[data-lvl="p2"]').first().click(); await pg.waitForTimeout(200);
await pg.locator('#heArbol button[data-lvl="p3"]').first().click(); await pg.waitForTimeout(250);
await pg.click('[data-act="guardarHE"]'); await pg.waitForTimeout(1400);
check('Bloquea una segunda solicitud viva el mismo día para la misma persona',
  /vigente|Ya hay/i.test(await pg.locator('#modalHEErr').textContent()||''),
  await pg.locator('#modalHEErr').textContent());
await pg.click('[data-act="cerrarModalHE"]'); await pg.waitForTimeout(300);
...
await pg.click('[data-act="abrirModalHE"]');
await pg.waitForSelector('#modalHE:not(.hidden)'); await pg.waitForTimeout(700);
await pg.selectOption('#fh_personal_sel','8');
await pg.click('[data-act="agregarPersonalHE"]'); await pg.waitForTimeout(250);

await pg.click('#heArbol button[data-lvl="cat"][data-val="recepcion"]'); await pg.waitForTimeout(300);
check('Pregunta nacional o internacional',
  (await pg.locator('#heArbol').textContent()).includes('nacional o internacional'));
await pg.locator('#heArbol button[data-lvl="p1"]').first().click(); await pg.waitForTimeout(300);
const txtProv = await pg.locator('#heArbol').textContent();
check('Aparece el paso del nombre del proveedor', txtProv.includes('nombre del proveedor'), txtProv.slice(-160));
check('Es un campo de texto libre', await pg.locator('#inp_p1b_heCr').isVisible());
/* Nombre con apóstrofo y comillas: antes esto rompía los onclick inline */
await pg.fill('#inp_p1b_heCr', 'Proveedor O\'Brien "El Norte" S.A. de C.V.');
await pg.locator('#heArbol button[data-act="setNivelInp"][data-lvl="p1b"]').click(); await pg.waitForTimeout(350);
check('Acepta el nombre del proveedor y continúa a los porqués',
  (await pg.locator('#heArbol').textContent()).includes('llegó fuera de horario'));
await pg.locator('#heArbol button[data-lvl="p2"]').first().click(); await pg.waitForTimeout(250);
await pg.locator('#heArbol button[data-lvl="p3"]').first().click(); await pg.waitForTimeout(350);
const resumen = await pg.locator('#heResumen').textContent();
...
await pg.locator('#heArbol button[data-act="reset"][data-lvl="p1b"]').click(); await pg.waitForTimeout(350);
check('El botón Cambiar del proveedor funciona con apóstrofos y comillas',
  await pg.locator('#inp_p1b_heCr').isVisible());
check('Al cambiar el proveedor se reinician los porqués siguientes',
  !(await pg.locator('#heResumen .causa-ok').isVisible()));
await pg.fill('#inp_p1b_heCr', 'Distribuciones «Ñandú» & Cía. <test>');
await pg.locator('#heArbol button[data-act="setNivelInp"][data-lvl="p1b"]').click(); await pg.waitForTimeout(350);
check('Acepta acentos, comillas latinas, & y signos de HTML',
  (await pg.locator('#heArbol').textContent()).includes('Ñandú'));
await pg.click('[data-act="cerrarModalHE"]'); await pg.waitForTimeout(300);
...
/* Se busca por folio: el nombre del jefe también aparece como
   "Solicitó:" en las tarjetas de su personal, así que buscar por
   nombre traía la tarjeta equivocada. */
const mia = pg.locator('.reg-card', { hasText: 'HE-MIO00001' }).first();
check('El jefe ve su propia solicitud en la lista', await mia.isVisible());
check('Se marca como MIS HORAS', (await mia.textContent()).includes('MIS HORAS'));
...
/* Ese registro es heredado de la v2.2 (origen 'auto'), así que todavía
   pasa por el nivel de jefe. Los que él mismo levanta ya vienen
   firmados en ese nivel y sólo esperan al trabajador. */
check('SÍ puede autorizar un registro heredado de su personal',
  await ajena.locator('button[data-act="autorizar"]').count() > 0);
const alTrabajador = pg.locator('.reg-card', { hasText: 'ESPERA AL TRABAJADOR' }).first();
check('Las que él levantó esperan al trabajador, no a él',
  await alTrabajador.count() > 0 &&
  await alTrabajador.locator('button[data-act="autorizar"]').count() === 0);
